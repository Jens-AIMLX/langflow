...
class FileDebugComponent(Component):
    display_name = "File Debug"
    description = "Debug component to examine FileInput properties"
    icon = "bug"
    name = "FileDebug"
    ...
    outputs = [
        Output(display_name="Debug Info", name="debug_info", method="debug_file"),
    ]

    def debug_file(self) -> Message:
        try:
            ...
            if hasattr(self, '_inputs'):
                ...
                if 'test_file' in self._inputs:
                    test_input = self._inputs['test_file']
                    debug_info.append(f"  test_input object: {test_input}")
                    debug_info.append(f"  test_input type: {type(test_input)}")

                    # Check ALL attributes (not just specific ones)
                    all_input_attrs = [attr for attr in dir(test_input) if not attr.startswith('_')]
                    debug_info.append(f"  test_input ALL attributes: {all_input_attrs}")

                    # Get values for ALL attributes
                    for attr in all_input_attrs:
                        try:
                            attr_value = getattr(test_input, attr)
                            if not callable(attr_value):  # Skip methods
                                debug_info.append(f"  test_input.{attr} = {attr_value}")
                        except Exception as e:
                            debug_info.append(f"  test_input.{attr} = ERROR: {e}")

                    # Check private attributes too
                    private_attrs = [attr for attr in dir(test_input) if attr.startswith('_') and not attr.startswith('__')]
                    debug_info.append(f"  test_input PRIVATE attributes: {private_attrs}")
                ...
                try:
                    if isinstance(obj, str):
                        ...
                        for key, value in obj.items():
                            search_object(value, f"{path}.{key}")
                    elif isinstance(obj, list):
                        for i, item in enumerate(obj):
                            search_object(item, f"{path}[{i}]")
                            ...
                            if not attr_name.startswith('__'):
                                try:
                                    ...
                                    if not callable(attr_value):
                                        search_object(attr_value, f"{path}.{attr_name}")
                                except:
                                    pass
                except:
                    pass
            ...
            return Message(
                text=final_text,
                sender="File Debug",
                sender_name="File Debug Component"
            )

        except Exception as e:
            error_msg = f"Error in file debug: {str(e)}"
